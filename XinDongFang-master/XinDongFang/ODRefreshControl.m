...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    if (_refreshing) {
        if (offset != 0) {
            ...
            _shapeLayer.position = CGPointMake(0, _maxDistance + offset + kOpenedViewHeight);
    ...
    CGFloat distance = MIN(_maxDistance, fabs(verticalShift));
    CGFloat percentage = 1 - (distance / _maxDistance);
    ...
    if (distance == 0) {
        ...
        if (percentage == 0) {
            bottomOrigin.y -= (fabs(verticalShift) - _maxDistance);
            triggered = YES;
        }
    }
